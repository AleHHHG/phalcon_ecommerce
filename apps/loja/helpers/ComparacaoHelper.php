...
class ComparacaoHelper extends BaseHelper{
	...
	protected function getItens($param){
		...
		foreach ($this->options['produtos'] as $key => $value) {
			...
			if($param == 'imagem'){
				$html .= '<img src="'.$this->url_base.'files/produtos/'.$value['imagens'][0].'" class="img-responsive" width="150px" />';
			}else if($param == 'valor' || $param == 'peso' || $param == 'dimensões'){
				if($param == 'valor'){
					if($this->ecommerce_options->produto_detalhes == '1'){
						$html .= 'R$ '.$value['detalhes'][0][$param];
					}else{
						$html .= 'R$ '.$value[$param];
					}
				...
				$html .= '<a href="javascript:;" class="btn btn-primary"><i class="fa fa-shopping-cart"></i></a> ';
